...
def create_ppmi_alpha_vectors(wids, o_counts, co_counts, tot_count, alpha):
    ...
    for o_count in o_counts:
        alpha_tot_count += (o_count/tot_count)**alpha
    ...
    for wid0 in wids:
        ...
        for co in co_counts[wid0]:
            vectors[wid0][co] = max(0, PMI(co_counts[wid0][co], o_counts[wid0], (o_counts[co]/tot_count)**alpha, alpha_tot_count))

    return vectors

def create_ppmi_laplace_vectors(wids, o_counts, co_counts, tot_count, alpha=2):
    '''Creates context vectors for the words in wids, using PPMI.
    These should be sparse vectors.
    ...
    vocab_size = len(o_counts)
    
    vectors = {}
    for w in wids:
        vectors[w] = {}
        
        p_x = o_counts[w]/tot_count
        ...
        for c in o_counts:
            ...
            p_xy = (c_xy + 2)/(tot_count + 2*vocab_size)
            
            pmi = log(p_xy/(p_x*p_y),2)
        
            vectors[w][c] = max(0, pmi)
