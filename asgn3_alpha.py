...
def create_ppmi_alpha_vectors_orig(wids, o_counts, co_counts, tot_count, alpha):
    '''Creates context vectors for the words in wids, using PPMI.
    These should be sparse vectors.
    ...
    for o_occur_id, count in o_counts.items():
        total_pow_alpha += int(count)**alpha
    ...
    for wid0 in wids:
        ...
        for co in co_counts[wid0]:
            vectors[wid0][co] = max(0, PMI(co_counts[wid0][co], o_counts[wid0], o_counts[co]**alpha, total_pow_alpha))
...
#vectors = create_ppmi_laplace_vectors(all_wids, o_counts, co_counts, N, 0.00000001)
vectors = create_ppmi_alpha_vectors(all_wids, o_counts, co_counts, N, 0.75)
#vectors = create_ppmi_alpha_vectors_orig(all_wids, o_counts, co_counts, N, 0.75)
