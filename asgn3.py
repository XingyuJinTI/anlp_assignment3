...
def distance(v0,v1,wid0,wid1,p):
  '''Compute the Eudlidean distance and Manhattan distance between two sparse vectors.
  ...
  :type wid0: int
  :type wid1: int
  :param wid0: first word ID
  :param wid1: second word ID
  :type p: int
  :param p: power of distance, Euclidean if p=2; Manhattan if p=1
  :rtype: float
  :return: distance between v0 and v1
  '''
  ids_in_both = {}
  ids_in_both = set()
  ...
  for wid in ids_in_both:
      ...
      square_distance += abs((v1_elem - v0_elem)**2)
        
  square_distance = square_distance**(1/p)
...
def create_ppmi_alpha_vectors(wids, o_counts, co_counts, tot_count, alpha):
    '''Creates context vectors for the words in wids, using PPMI.
    These should be sparse vectors.
    ...
    for wid0 in wids:
        ...
        for co in co_counts[wid0]:
            vectors[wid0][co] = max(0, PMI(co_counts[wid0][co], o_counts[wid0], o_counts[co]**alpha, tot_count**alpha))
...
#original 5 test words
#test_words = ["cat", "dog", "mouse", "computer","@justinbieber"]
#100 randomly selected words and make pairs by all possible combinations
...
'''
# 27 selected word pairs
test_word_pairs = [["jersey", "shirt"], ["eraser", "rubber"], ["revise", "study"], ["pants", "trousers"], ["diaper", "nappy"], ["boot", "cleat"], ["vacuum", "hoover"], ["eggplant", "aubergine"], ["zucchini", "courgette"], ["dog", "cat"], ["yellow", "purple"], ["coffee", "lemonade"], ["london", "swindon"], ["california", "alaska"], ["penguin", "kangaroo"], ["settee", "curtain"], ["hairspray", "wristband"], ["zinc", "potassium"], ["traffic", "score"], ["cookie", "wind"], ["popular", "park"], ["colour", "traitor"], ["entry", "cardboard"], ["replace", "astronomical"], ["loove", "antelop"], ["toothbrush", "diplomat"], ["swaziland", "wretched"]]
...
#make the word vectors by PPMI or PPMI-alpha
vectors = create_ppmi_vectors(all_wids, o_counts, co_counts, N)
#vectors = create_ppmi_alpha_vectors(all_wids, o_counts, co_counts, N, 0.75)

#compute cosine similarites or Euclidean distance for all pairs we consider
sims = {(wid0,wid1): cos_sim(vectors[wid0],vectors[wid1]) for (wid0,wid1) in wid_pairs}
#sims = {(wid0,wid1): distance(vectors[wid0],vectors[wid1],wid0,wid1,2) for (wid0,wid1) in wid_pairs}

freq_v_sim(sims)

print("Sort by similarity")
print_sorted_pairs(sims, o_counts)
